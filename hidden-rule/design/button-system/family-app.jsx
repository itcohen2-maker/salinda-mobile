...
function HRIconButton({ label = "Hint", icon = "hint", state = "idle", compact, large, onClick }) {
  const cls = [
    "hr-icon-button",
    compact && "hr-icon-button--compact",
    large && "hr-icon-button--large",
    state === "pressed" && "is-pressed",
    state === "focused" && "is-focused",
    state === "selected" && "is-selected",
    state === "disabled" && "is-disabled",
  ...
  return (
    <button type="button" className={cls} disabled={state === "disabled"}
            aria-label={label} aria-pressed={state === "selected" || undefined}
            onClick={onClick}>
      <span className="hr-icon-button__face">
        <span className="hr-icon-button__glyph">
          <IconGlyph icon={icon} />
        </span>
      </span>
    </button>
...
function HRInput({
  ...
  value = "",
  placeholder = "Type the rule you see",
  hint = "Keep it short. You can revise before confirming.",
  state = "idle",
  prefixIcon = "search",
  suffix,
  block,
  ...
  return (
    <div className={cls} lang={lang} dir="auto">
      <div className="hr-input__chrome">
        <div className="hr-input__eyebrow" lang={lang} dir="auto">{label}</div>
        <div className="hr-input__field-row">
          <span className="hr-input__slot" aria-hidden="true">
            <IconGlyph icon={prefixIcon} />
          </span>
          <input
            className="hr-input__control"
            value={value}
          ...
          />
          {suffix ? <span className="hr-input__suffix">{suffix}</span> : null}
        </div>
      </div>
      {hint ? <div className="hr-input__hint">{hint}</div> : null}
    </div>
  );
}
...
function IconUtilityBoard() {
  ...
  return (
    <Backdrop tone="charcoal" style={{ flexDirection: "column", gap: 18, padding: 30 }}>
      <ComponentTag name="Icon button" />
      <div style={{
        display: "grid",
        ...
        {items.map(([label, icon]) => (
          <div key={icon} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
            <HRIconButton icon={icon} label={label} />
            <div style={{
              fontFamily: '"Frank Ruhl Libre", Georgia, serif',
              fontSize: 11,
              letterSpacing: "0.16em",
              textTransform: "uppercase",
              color: "rgba(214, 179, 108, 0.68)",
              fontWeight: 500,
            }}>{label}</div>
          </div>
        ))}
      </div>
      <Caption>Quiet utility controls · same matte language as Secondary</Caption>
    </Backdrop>
  );
}
...
function IconStatesBoard() {
  ...
  return (
    <Backdrop tone="charcoal" style={{ flexDirection: "column", gap: 18, padding: 30 }}>
      ...
      <Caption>Selected uses the same ink-blue selection logic as Secondary</Caption>
    </Backdrop>
  );
}

function InputStatesBoard() {
  return (
    <Backdrop tone="charcoal" style={{ flexDirection: "column", gap: 14, padding: 28, paddingTop: 44 }}>
      <ComponentTag name="Input field" />
      <HRInput
        label="Working Theory"
        placeholder="Type the rule you see"
        hint="Idle · ready for first entry."
      />
      <HRInput
        label="Working Theory"
        placeholder="Type the rule you see"
        hint="Focused · brass eyebrow stays restrained."
        state="focused"
      />
      <HRInput
        label="Working Theory"
        value="Pairs that sum to ten"
        hint="Filled · the field reads as a quiet editorial note."
        state="filled"
      />
      <HRInput
        label="Working Theory"
        value="Only red cards"
        hint="That wording conflicts with the revealed examples."
        state="invalid"
        suffix="revise"
      />
      <HRInput
        label="Working Theory"
        placeholder="Locked during reveal"
        hint="Disabled · no decorative glow."
        state="disabled"
      />
      <Caption>Idle · focused · filled · invalid · disabled</Caption>
    </Backdrop>
  );
}

function PhoneRuleNamingBoard() {
  return (
    <Backdrop tone="charcoal" style={{ padding: 40 }}>
      <PhoneShell>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
          <HRIconButton icon="back" label="Back" />
          <div style={{
            fontFamily: '"Frank Ruhl Libre", Georgia, serif',
            fontSize: 12,
          ...
          <HRIconButton icon="rules" label="History" />
        </div>
        ...
        <div style={{
          ...
          textAlign: "center",
          marginBottom: 14,
        }}>The accepted cards share a relationship the rejected cards do not.</div>
        ...
        <div style={{ marginTop: 12 }}>
          <HRInput
            ...
            label="Working Theory"
            placeholder="Type the rule you see"
            hint="You can refine this before confirming."
            state="focused"
            suffix="draft"
          />
        </div>
      ...
      <Caption>Controls in context · rule naming flow</Caption>
    </Backdrop>
  );
}
...
function App() {
  return (
    <DesignCanvas storageKey="hidden-rule-family-v2-1">
      ...
      <DCSection id="supporting-controls" title="Supporting controls">
        <DCArtboard id="icon-utility" label="Icon button · utility set" width={460} height={300}>
          <IconUtilityBoard />
        </DCArtboard>
        <DCArtboard id="icon-states" label="Icon button · states" width={520} height={300}>
          <IconStatesBoard />
        </DCArtboard>
        <DCArtboard id="input-states" label="Input field · states" width={420} height={760}>
          <InputStatesBoard />
        </DCArtboard>
        <DCArtboard id="rule-naming" label="Rule naming · in context" width={420} height={760}>
          <PhoneRuleNamingBoard />
        </DCArtboard>
      </DCSection>
