...
/* ------------------------------------------------------------------ */
/*  Secondary button — quieter sibling                                 */
/* ------------------------------------------------------------------ */
function HRButton2({ label = "Hint", state = "idle", block, narrow, onClick }) {
  const cls = [
    "hr-btn2",
    block && "hr-btn2--block",
    narrow && "hr-btn2--narrow",
    state === "pressed"  && "is-pressed",
  ...
  return (
    <button
      ...
      aria-label={label}
    >
      <span className="hr-btn2__label">{label}</span>
    </button>
  );
}
window.HRButton2 = HRButton2;
...
function SecondaryStateArtboard({ state, label = "Hint", tone = "charcoal" }) {
  const dark = tone !== "parchment";
  return (
    <Backdrop tone={tone}>
      <HRButton2 label={label} state={state} />
      <Caption dark={dark}>{state}</Caption>
    </Backdrop>
  );
}

function SecondaryLabelVariantsArtboard() {
  const labels = ["Hint", "Back", "Rules", "Continue", "Cancel"];
  return (
    <Backdrop tone="charcoal" style={{ flexDirection: "column", gap: 16, padding: 32 }}>
      {labels.map(l => <HRButton2 key={l} label={l} />)}
      <Caption dark>Dynamic text · variable widths</Caption>
    </Backdrop>
  );
}

/* — Family · primary + secondary together, to confirm cohesion — */
function FamilyPairArtboard() {
  return (
    <Backdrop tone="charcoal" style={{ flexDirection: "column", gap: 28, padding: 32 }}>
      <HRButton label="Name the Rule" />
      <div style={{ display: "flex", gap: 14 }}>
        <HRButton2 label="Hint" />
        <HRButton2 label="Rules" state="selected" />
        <HRButton2 label="Back" narrow />
      </div>
      <Caption dark>Family · primary + secondary</Caption>
    </Backdrop>
  );
}

/* — A second in-context demo: a "Rules" screen with both buttons — */
function RulesScreenArtboard() {
  return (
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 22 }}>
            <HRButton2 label="Basics" state="selected" narrow />
            <HRButton2 label="Suits" narrow />
            <HRButton2 label="Scoring" narrow />
          </div>
          ...
          <div style={{
            ...
            <p>Name the rule precisely. Approximate answers are returned to the deck.</p>
          </div>

          {/* Footer actions */}
          <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
            <HRButton2 label="Hint" block />
            <div style={{ flex: 1 }}><HRButton label="Continue" /></div>
          </div>
        </div>
      </div>
      <Caption dark>Rules screen · both buttons</Caption>
    </Backdrop>
  );
}

/* — Secondary playground — */
function SecondaryPlaygroundArtboard() {
  const [label, setLabel] = useState("Hint");
  const [state, setState] = useState("idle");
  const [tone, setTone] = useState("charcoal");

  const stateOptions = ["idle", "pressed", "selected", "disabled"];
  const toneOptions = [
    ["parchment", "Parchment"],
    ["charcoal", "Charcoal"],
    ["ink", "Ink"],
...
/* ------------------------------------------------------------------ */
function App() {
  const V2 = window.V2Sections;
  return (
    <DesignCanvas storageKey="hidden-rule-cta-v2">
      {V2 ? <V2 /> : null}

      <DCSection id="states-light" title="v1 · States · on parchment">
      ...
      <DCSection id="states-dark" title="v1 · States · on charcoal">
      ...
      <DCSection id="anatomy" title="v1 · Anatomy & safe area">
      ...
      <DCSection id="context" title="v1 · In context · primary">
      ...
      <DCSection id="secondary-states" title="v1 · Secondary · states">
        <DCArtboard id="sec-idle"     label="Idle"     width={340} height={220}>
          <SecondaryStateArtboard state="idle" label="Hint" />
        </DCArtboard>
        <DCArtboard id="sec-pressed"  label="Pressed"  width={340} height={220}>
          <SecondaryStateArtboard state="pressed" label="Hint" />
        </DCArtboard>
        <DCArtboard id="sec-selected" label="Selected" width={340} height={220}>
          <SecondaryStateArtboard state="selected" label="Rules" />
        </DCArtboard>
        <DCArtboard id="sec-disabled" label="Disabled" width={340} height={220}>
          <SecondaryStateArtboard state="disabled" label="Hint" />
        </DCArtboard>
      </DCSection>

      <DCSection id="secondary-meta" title="v1 · Secondary · variants & family">
        <DCArtboard id="sec-labels" label="Dynamic text" width={340} height={440}>
          <SecondaryLabelVariantsArtboard />
        </DCArtboard>
        <DCArtboard id="family-pair" label="With the primary" width={420} height={440}>
          <FamilyPairArtboard />
        </DCArtboard>
        <DCArtboard id="sec-playground" label="Playground · live" width={520} height={440}>
          <SecondaryPlaygroundArtboard />
        </DCArtboard>
      </DCSection>

      <DCSection id="rules-context" title="v1 · In context · together">
        <DCArtboard id="rules-screen" label="Rules screen" width={420} height={760}>
          <RulesScreenArtboard />
        </DCArtboard>
      </DCSection>
    </DesignCanvas>
